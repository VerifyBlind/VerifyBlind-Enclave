...
public class EnclaveServiceTests
{
    ...
    /// <summary>
    /// Süre aşımı AYRI kodla (ERR_NONCE_EXPIRED) raporlanmalı: relay bu kodu beklenen kullanıcı
    /// durumu sayıp Sentry'ye event üretmeyen seviyede logluyor. Kod ERR_NONCE_VERIFICATION'a geri
    /// düşerse geç kalan her kullanıcı yeniden uyarı (ve Sentry kotası) üretir.
    /// </summary>
    [Fact]
    public async Task RegisterAsync_NonceExpired_ThrowsNonceExpiredWithAgeDiagnostic()
    {
        var now = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        var request = BuildRequest(
            nonce: "test-nonce",
            timestamp: now - 1000, // 16+ dakika önce — süresi dolmuş (limit 900s)
            nonceSignature: "sig");
        ...
        Assert.Equal("ERR_NONCE_EXPIRED", ex.ErrorCode);
        Assert.StartsWith("nonce-expired age=", ex.Diagnostic);
        Assert.Contains("max=900s", ex.Diagnostic);
    }
    ...
    public async Task RegisterAsync_NonceSignatureInvalid_ThrowsNonceVerification()
    {
        ...
        Assert.Equal(RegistrationStep.NonceVerification, ex.Step);
        // İmza geçersizliği gerçek anomalidir — süre aşımıyla aynı kodu PAYLAŞMAZ, uyarı üretmeye
        // devam etmesi gerekir.
        Assert.Equal("ERR_NONCE_VERIFICATION", ex.ErrorCode);
    }
    ...
    [Fact]
    public void VerifyNonce_ExpiredTimestamp_ThrowsNonceExpired()
    {
        var old = DateTimeOffset.UtcNow.ToUnixTimeSeconds() - 1000; // 1000s > 900s limiti
        var payload = new SecurePayload { Nonce = "n", Timestamp = old, NonceSignature = "sig" };

        var ex = Assert.Throws<NonceExpiredException>(() => _service.VerifyNonce(payload));

        Assert.Equal(900, ex.MaxAgeSeconds);
        Assert.InRange(ex.AgeSeconds, 1000, 1010);
    }

    /// <summary>
    /// İleri cihaz saati süre aşımı DEĞİLDİR: gerçek anomalidir (saat kayması) ve süre aşımı gibi
    /// sessizleştirilmemelidir. Assert.Throws tip-kesin olduğu için bu test aynı zamanda
    /// NonceExpiredException'a düşmediğini de garanti eder.
    /// </summary>
    [Fact]
    public void VerifyNonce_FutureTimestamp_ThrowsPlainInvalidOperation()
    {
    ...
    public void VerifyActiveAuth_BothDg15AndActiveSigMissing_Throws()
    {
        ...
        Assert.ThrowsAny<Exception>(() => _service.VerifyActiveAuth(payload));
    }
    ...
    // ── VerifyActiveAuth — GERÇEK ICAO AA imzası (ISO 9796-2 Şema 1) ──────────────
    // Gerçek çipler PKCS#1 KULLANMAZ. INTERNAL AUTHENTICATE çıktısı:
    //     F = 0x6A || M1 || Hash(M1 ‖ challenge) || Trailer
    // M1 çipin rastgelesidir (RND.ICC) ve challenge M1 İÇİNDE YER ALMAZ — yalnız hash girdisidir.
    // Sentry VERIFYBLIND-API-E (2026-08-21): iOS istemcisi imzayı DG15 ile YEREL doğrularken
    // (AApassed=true) enclave aynı imzayı "İmza formatı tanınmıyor" diye reddetti.

    private const int AaKeyBits = 1536; // TC kimlik kartında gözlenen boyut (DG15=227B, imza=192B)

    /// Bir çipin ürettiği ISO 9796-2 Şema 1 (kısmi kurtarma) AA imzasını birebir üretir.
    private static (string Dg15, string Sig, string Challenge) BuildIcaoAaSignature(
        string nonce, string digestName, byte[] trailer)
    {
        ...
        gen.Init(new Org.BouncyCastle.Crypto.Parameters.RsaKeyGenerationParameters(
            Org.BouncyCastle.Math.BigInteger.ValueOf(65537), rnd, AaKeyBits, 100));
        var pair = gen.GenerateKeyPair();
        var pub = (Org.BouncyCastle.Crypto.Parameters.RsaKeyParameters)pair.Public;
        var priv = (Org.BouncyCastle.Crypto.Parameters.RsaKeyParameters)pair.Private;

        // DG15 = 0x6F | uzunluk | SubjectPublicKeyInfo
        var spki = Org.BouncyCastle.X509.SubjectPublicKeyInfoFactory
            .CreateSubjectPublicKeyInfo(pub).GetDerEncoded();
        var dg15 = new byte[] { 0x6F, 0x81, (byte)spki.Length }.Concat(spki).ToArray();

        var challenge = SHA256.HashData(Encoding.UTF8.GetBytes(nonce)).Take(8).ToArray();

        var digest = Org.BouncyCastle.Security.DigestUtilities.GetDigest(digestName);
        int blockLen = (AaKeyBits - 1 + 7) / 8;
        int hashLen = digest.GetDigestSize();
        int m1Len = blockLen - 1 - hashLen - trailer.Length;

        var m1 = new byte[m1Len];
        rnd.NextBytes(m1); // çipin RND.ICC'si

        var hash = new byte[hashLen];
        digest.BlockUpdate(m1, 0, m1.Length);
        digest.BlockUpdate(challenge, 0, challenge.Length);
        digest.DoFinal(hash, 0);

        var block = new byte[blockLen];
        block[0] = 0x6A;
        Buffer.BlockCopy(m1, 0, block, 1, m1Len);
        Buffer.BlockCopy(hash, 0, block, 1 + m1Len, hashLen);
        Buffer.BlockCopy(trailer, 0, block, 1 + m1Len + hashLen, trailer.Length);

        // Ham RSA: s = block^d mod n  (çipin döndürdüğü imza)
        var s = new Org.BouncyCastle.Math.BigInteger(1, block).ModPow(priv.Exponent, priv.Modulus);
        var sig = Org.BouncyCastle.Utilities.BigIntegers.AsUnsignedByteArray(AaKeyBits / 8, s);

        return (Convert.ToBase64String(dg15), Convert.ToBase64String(sig), Convert.ToBase64String(challenge));
    }

    [Theory]
    // Trailer 0x34CC = SHA-256, 0x33CC = SHA-1 (açık); 0xBC = örtük (hash bağlamdan bilinir).
    [InlineData("SHA-256", new byte[] { 0x34, 0xCC })]
    [InlineData("SHA-1", new byte[] { 0x33, 0xCC })]
    [InlineData("SHA-1", new byte[] { 0xBC })]
    [InlineData("SHA-256", new byte[] { 0xBC })]
    public void VerifyActiveAuth_GenuineIcaoChipSignature_DoesNotThrow(string digestName, byte[] trailer)
    {
        const string nonce = "icao-aa-nonce-42";
        var (dg15, sig, challenge) = BuildIcaoAaSignature(nonce, digestName, trailer);
        ...
        _service.VerifyActiveAuth(payload); // gerçek çip imzası — reddedilmemeli
    }

    [Fact]
    public void VerifyActiveAuth_GenuineSignatureWithWrongChallenge_Throws()
    {
        // Aynı yapı, ama imza BAŞKA bir challenge üzerinde üretildi → reddedilmeli
        // (doğrulamanın gevşetilmesi bu güvenlik özelliğini bozmamalı).
        const string nonce = "icao-aa-nonce-42";
        var (dg15, sig, _) = BuildIcaoAaSignature("baska-bir-nonce", "SHA-256", new byte[] { 0x34, 0xCC });
        var challenge = SHA256.HashData(Encoding.UTF8.GetBytes(nonce)).Take(8).ToArray();
    ...
    // ── AA teşhis profili — kart elimizde OLMADAN hata ayıklayabilmenin tek yolu ──────────
    // Üretim enclave'i debug modda değil; Console çıktısı görünmez ve kullanıcıdan kartını tekrar
    // denemesini isteyemeyiz. Bu yüzden her AA sonucu YAPISAL bir profil taşımak zorunda.

    [Fact]
    public void VerifyActiveAuth_Success_ReturnsProfileNamingTheVariant()
    {
        const string nonce = "aa-profile-ok";
        var (dg15, sig, challenge) = BuildIcaoAaSignature(nonce, "SHA-256", new byte[] { 0x34, 0xCC });

        var profile = _service.VerifyActiveAuth(new SecurePayload
        {
            Nonce = nonce, DG15 = dg15, ActiveSig = sig, AAChallenge = challenge
        });

        Assert.Contains("alg:RSA", profile);
        Assert.Contains($"bits:{AaKeyBits}", profile);
        Assert.Contains("match:iso9796-2/SHA-256/explicit", profile);
    }

    [Fact]
    public void VerifyActiveAuth_UnrecognizedSignature_ProfileNamesSchemeBytes()
    {
        // Geçerli DG15, ama hiçbir varyantın tutmadığı bir imza → profil, hangi şemanın
        // kullanıldığını söyleyecek blok başlığı/trailer'ını taşımalı.
        const string nonce = "aa-profile-fail";
        var (dg15, _, challenge) = BuildIcaoAaSignature(nonce, "SHA-256", new byte[] { 0x34, 0xCC });

        var bogus = new byte[AaKeyBits / 8];
        new Random(42).NextBytes(bogus);
        bogus[0] = 0x01; // modülüsün altında kalsın ki RSA çözümü çalışsın

        var ex = Assert.Throws<ActiveAuthException>(() => _service.VerifyActiveAuth(new SecurePayload
        {
            Nonce = nonce, DG15 = dg15, ActiveSig = Convert.ToBase64String(bogus), AAChallenge = challenge
        }));

        Assert.NotNull(ex.Profile);
        Assert.Contains("alg:RSA", ex.Profile);
        Assert.Contains($"bits:{AaKeyBits}", ex.Profile);
        Assert.Contains("hdr:", ex.Profile);   // ISO 9796-2 başlık baytı
        Assert.Contains("tr:", ex.Profile);    // trailer baytları → hash kimliği
        Assert.Contains("match:none", ex.Profile);
    }

    [Fact]
    public void VerifyActiveAuth_Profile_NeverCarriesCardIdentifyingData()
    {
        // Açık anahtar ve imza karta ÖZGÜDÜR; profile girerlerse loglar üzerinden kart-kayıt
        // ilişkilendirmesi mümkün olur. Profil yalnız yapısal alan taşımalı.
        const string nonce = "aa-profile-pii";
        var (dg15, sig, challenge) = BuildIcaoAaSignature(nonce, "SHA-1", new byte[] { 0xBC });

        var okProfile = _service.VerifyActiveAuth(new SecurePayload
        {
            Nonce = nonce, DG15 = dg15, ActiveSig = sig, AAChallenge = challenge
        });

        foreach (var secret in new[] { dg15, sig, challenge })
        {
            Assert.DoesNotContain(secret, okProfile);
            Assert.DoesNotContain(secret[..Math.Min(16, secret.Length)], okProfile);
        }
        // Profil kısa ve sabit formatlı kalmalı — serbest metin sızıntısı olmasın.
        Assert.True(okProfile.Length < 120, $"profil beklenenden uzun: {okProfile}");
    }
    ...
    public void VerifyDGHashes_Dg1HashNotInSod_Throws()
    {
        ...
        Assert.Throws<PassiveAuthException>(() => PassiveAuth.VerifyDGHashes(sod, dg1, dg2, null!));
    }
    ...
    public void VerifyDGHashes_MissingRawDg2_Throws()
    {
        ...
        var dg1 = BuildDG1Base64(FakeTurkishTD1Mrz);
        var sod = ScanSod(SHA256.HashData(Convert.FromBase64String(dg1)));
        var ex = Assert.Throws<PassiveAuthException>(() => PassiveAuth.VerifyDGHashes(sod, dg1, "", null!));
    ...
    public void VerifyDGHashes_Dg15HashMissing_Throws()
    {
        ...
        Assert.Throws<PassiveAuthException>(() =>
            PassiveAuth.VerifyDGHashes(sod, dg1, dg2, Convert.ToBase64String(new byte[] { 0x11, 0x22 })));
    }

    /// Pasif doğrulama hataları AA ile aynı muameleyi görmeli: kart elimizde olmadan hangi kontrolün
    /// düştüğü tek satırdan anlaşılmalı. CSCA deposu eksikliği (kurulum hatamız), CRL bayatlığı,
    /// DG hash uyuşmazlığı (NFC bozulması) ve iptal (gerçek güvenlik olayı) çok farklı sonuçlar
    /// doğurur — ayırt edilemezlerse hiçbiri araştırılamaz.
    [Fact]
    public void VerifyDGHashes_Failure_CarriesStructuralProfile()
    {
        var dg1 = BuildDG1Base64(FakeTurkishTD1Mrz);
        var dg2 = Convert.ToBase64String(new byte[] { 0xD2, 0xD2 });
        var sod = ScanSod(SHA256.HashData(Convert.FromBase64String(dg1)),
        ...
        var ex = Assert.Throws<PassiveAuthException>(() =>
            PassiveAuth.VerifyDGHashes(sod, dg1, dg2, Convert.ToBase64String(new byte[] { 0x11, 0x22 })));

        Assert.NotNull(ex.Profile);
        Assert.StartsWith("pa:", ex.Profile);       // hangi kontrol düştü
        Assert.Contains("dg:15", ex.Profile);       // hangi veri grubu
        Assert.True(ex.Profile!.Length < 120, $"profil beklenenden uzun: {ex.Profile}");
    }
