...
// 1. Get Country from DG1 to find correct CSCA folder
        ...
        if (certs.Count == 0)
            throw new PassiveAuthException($"Çip doğrulama yapılamadığından bu kart desteklenmemektedir ({countryCode} için CSCA sertifikası bulunamadı).")
            { Profile = $"pa:csca-missing,country:{countryCode}" };
...
try
        ...
        catch (System.Security.Cryptography.CryptographicException cex)
        {
            ...
            throw new PassiveAuthException($"Pasif Kimlik Doğrulama Başarısız: SOD yapısı çözümlenemedi. ({cex.Message})")
            { Profile = $"pa:sod-parse,sod:{sodBytes.Length}B,ex:{cex.GetType().Name}" };
        } 
...
try
            ...
            catch (System.Security.Cryptography.CryptographicException cex)
            {
                ...
                // İmzacının digest algoritması yapısal bir kuşak göstergesidir; sertifika KONUSU
                // (Subject) loglanmaz — o belge partisini tanımlar ve gereksizdir.
                var sigDigest = signedCms.SignerInfos.Count > 0
                    ? signedCms.SignerInfos[0].DigestAlgorithm.Value ?? "?"
                    : "none";
                throw new PassiveAuthException($"Pasif Kimlik Doğrulama Başarısız: SOD imza doğrulaması başarısız. ({cex.Message})")
                { Profile = $"pa:sod-signature,digest:{sigDigest},signers:{signedCms.SignerInfos.Count}" };
            } 
            ...
            if (dsCert == null)
                throw new PassiveAuthException("SOD içinde sertifika yok.")
                { Profile = "pa:ds-cert-missing" };
...
Console.WriteLine("[Enclave] ORS Zincir İnşası başarısız. Durumlar kontrol ediliyor...");
            ...
            if (!trusted)
            {
                ...
                throw new PassiveAuthException($"Çip doğrulama yapılamadığından bu kart desteklenmemektedir (kart sertifikası güvenilir CSCA ile doğrulanamadı).")
                { Profile = $"pa:chain-untrusted,country:{countryCode},csca:{certs.Count}" };
            }
...
Console.WriteLine($"[Enclave] SOD Doğrulaması Başarısız: {ex}");
            ...
            throw new PassiveAuthException($"SOD Zincir Doğrulaması Başarısız: {ex.Message}")
            { Profile = $"pa:chain-error,country:{countryCode},ex:{ex.GetType().Name}" };  
        }
    ...
    private static string VerifyOneDgHash(byte[] sodContent, LdsSecurityObject? lds, int dgNumber, string dgBase64, bool required)
    {
        ...
        {
            if (required)
                throw new PassiveAuthException($"DG{dgNumber} eksik — SOD bağlama doğrulaması yapılamıyor (istemci ham DG{dgNumber} baytlarını göndermeli).")
                { Profile = $"pa:dg-missing,dg:{dgNumber}" };
            return "absent";
        }

        byte[] dgBytes;
        try { dgBytes = Convert.FromBase64String(dgBase64); }
        catch { throw new PassiveAuthException($"DG{dgNumber} Base64 çözümlenemedi.") { Profile = $"pa:dg-decode,dg:{dgNumber}" }; }
        ...
        if (lds != null && lds.Hashes.TryGetValue(dgNumber, out var expected))
        {
            ...
            throw new PassiveAuthException($"DG{dgNumber} Hash Uyuşmazlığı! Hesaplanan hash, SOD'daki DG{dgNumber} slotuyla eşleşmiyor ({lds.DigestAlgorithm}). Belge ile bu veri grubu uyuşmuyor.")
            { Profile = $"pa:dg-hash,dg:{dgNumber},alg:{lds.DigestAlgorithm},len:{dgBytes.Length}B" };
        }
        ...
        throw new PassiveAuthException($"DG{dgNumber} Hash Uyuşmazlığı! (SHA-1/256/384/512 ile SOD'da eşleşme bulunamadı). NFC okuma sırasında veri bozulması olabilir.")
        { Profile = $"pa:dg-hash-nomatch,dg:{dgNumber},len:{dgBytes.Length}B" };
    }
...
private static void CheckCertificateRevocation(System.Security.Cryptography.X509Certificates.X509Certificate2 dsCert, string countryCode)
    {
        ...
        {
            // ⚠️ GERÇEK GÜVENLİK OLAYI — CRL bayatlığından ayırt edilebilmeli. Seri numarası
            // belge PARTİSİNİ tanımlar (kişiyi değil) ama yine de maskelenir; iptal vakasını
            // araştırmak için ilk 8 karakter yeterli.
            throw new PassiveAuthException($"Sertifika İptal Edilmiş! DS sertifikası CRL'de iptal edilmiş olarak işaretli. Bu belgenin imzası artık güvenilir değil.")
            { Profile = $"pa:crl-revoked,country:{countryCode},ds:{(dsSerial.Length > 8 ? dsSerial[..8] : dsSerial)}.." };
        }
