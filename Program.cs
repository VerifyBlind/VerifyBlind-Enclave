...
// Attestation belgesini trafiksiz saatlerde de taze tutar — belge tembel üretildiği için
// istek gelmeyen bir gece onu öldürüyordu (bkz. AttestationRefreshService).
builder.Services.AddHostedService<VerifyBlind.Enclave.Services.AttestationRefreshService>();

// Relay'in bildirdiği saat — enclave'in kayan saatine karşı düzeltme (bkz. RelayClock).
builder.Services.AddSingleton<VerifyBlind.Enclave.Services.RelayClock>();

var app = builder.Build();

// X-Relay-Time başlığını oku. Relay güvenilmeyen bir bileşen ama bu değer saati YALNIZCA
// ileri taşıyabiliyor (RelayClock.EffectiveUtcNow = max), dolayısıyla kötüye kullanımı
// fazladan yenilemeden ibarettir — süresi dolmuş belge servis ettiremez.
app.Use(async (ctx, next) =>
{
    if (ctx.Request.Headers.TryGetValue("X-Relay-Time", out var raw) &&
        DateTimeOffset.TryParse(raw.ToString(), System.Globalization.CultureInfo.InvariantCulture,
                                System.Globalization.DateTimeStyles.RoundtripKind, out var relayNow))
    {
        ctx.RequestServices.GetRequiredService<VerifyBlind.Enclave.Services.RelayClock>().Report(relayNow);
    }
    await next();
});
...
// Attestation/saat teşhisi — 2026-08-25 olayının görülebilir olması için.
// O gün enclave süresi dolmuş bir sertifikayla belge servis etti ve DIŞARIDAN bunu anlamanın
// hiçbir yolu yoktu: sağlık ucu yalnız "up" diyordu. Alarm da bu uçtan beslenir.
app.MapGet("/health/attestation", (VerifyBlind.Enclave.Services.IEnclaveKeyService keys,
                                   VerifyBlind.Enclave.Services.RelayClock clock) =>
{
    var now = DateTime.UtcNow;
    var notAfter = keys.AttestationNotAfter;
    return Results.Ok(new
    {
        enclave_utc_now = now.ToString("o"),
        relay_utc_reported = clock.LastReported?.UtcDateTime.ToString("o"),
        // Pozitif = enclave saati relay'in GERİSİNDE. Büyümesi saat kayması demektir.
        drift_behind_relay_seconds = clock.DriftBehindRelay?.TotalSeconds,
        attestation_not_after = notAfter?.UtcDateTime.ToString("o"),
        // Gerçek-zamana göre kalan ömür; negatifse belge ÖLÜ ve istemciler açılmıyor demektir.
        attestation_valid_for_seconds = notAfter.HasValue
            ? (notAfter.Value.UtcDateTime - clock.EffectiveUtcNow).TotalSeconds
            : (double?)null,
        attestation_refresh_at = keys.AttestationRefreshAt == DateTime.MinValue
            ? null : keys.AttestationRefreshAt.ToString("o")
    });
});
